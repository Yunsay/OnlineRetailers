<template>
  <div class="featured">
    <div class="featured-list">
      <ul>
        <li class="item" v-for="(item, index) in featureds">
          <div class="card">
            <div class="card-img">
...
<script>
  import {FeaturedCommodity} from 'api/featured'
  ...
  export default {
    ...
    created() {
      this._FeaturedCommodity()
    },
    methods: {
      // item.rightOne.price.offeredPrice.text
      // item.rightTwo.price.offeredPrice.text
      // item.rightTwo.price.pricingMessage.text
      _FeaturedCommodity() {
        FeaturedCommodity().then((res) => {
            this.featureds = res.cards
        })
      },
      price1(index) {
        let item = this.featureds[index];
